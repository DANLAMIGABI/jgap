 ...
 * @since 1.0
 */
public class OLD_MyIntegerGene
   ...
   * @since 3.0
   */
  public OLD_MyIntegerGene(final Configuration a_config)
      throws InvalidConfigurationException {
   ...
   * @since 2.0
   */
  public OLD_MyIntegerGene(final Configuration a_config, final int a_lowerBounds,
  ...
  protected Gene newGeneInternal() {
    try {
      OLD_MyIntegerGene result = new OLD_MyIntegerGene(getConfiguration(), m_lowerBounds,
          m_upperBounds);
      return result;
  ...
  public void setToRandomValue(final RandomGenerator a_numberGenerator) {
	  boolean exit = false;
		int i =-1;
		while(!exit){
			i = Math.abs(a_numberGenerator.nextInt(provider.length));
			if(Main.test[i] == false){
				setAllele(provider[i].getID());
				Main.test[provider[i].getID()] = true;
				exit = true;			
			}
		}
		for(int j=0; j<Main.test.length; j++){
			exit = exit && Main.test[provider[j].getID()];
		}
		if(exit){
			for(int j=0; j< Main.test.length; j++){
				Main.test[provider[j].getID()] = false;
			}
		}
  }
